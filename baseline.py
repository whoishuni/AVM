...
class SmoothingPreviewDialog(QDialog):
    ...
    def __init__(self, base_image: np.ndarray, initial_level: int, parent=None):
        ...
        """
        super().__init__(parent)
        self.setWindowTitle("YC Smoothing Preview")
...
class StepViewerDialog(QDialog):
    ...
    def __init__(self, steps: list, main_window: 'VesselTracerApp', parent=None):
        ...
        """
        super().__init__(parent)
        self.setWindowTitle("YC Processing Steps")
...
class PlotlyViewerDialog(QDialog):
    ...
    def __init__(self, html_content: str, parent=None):
        super().__init__(parent)
        self.setWindowTitle("YC 3D View")
...
class VesselTracerApp(QMainWindow):
    ...
    def show_3d_view(self):
        ...
        fig.update_layout(
            title_text='YC 3D Vessel Reconstruction',
            scene=dict(
    ...
    def start_analysis(self):
        ...
        self.show_full_analysis_steps(final_mask, mask_pixels, pathfinding_costmap, cost_map)
    ...
    def replay_path_animation(self, anim_data: dict):
        ...
        temporal_cost_map = anim_data["temporal_cost_map"]
        ...
        for i in range(len(pixels) - 1):
            ...
            segment = self.find_path_astar(current_costmap, start_node, end_node, identity_map,
                                           width_map, main_vessel_width, temporal_cost_map, viz_callback=update_visualization)
            if segment is None:
    ...
    def show_full_analysis_steps(self, final_mask, mask_pixels, pathfinding_costmap, temporal_cost_map):
        ...
        for _ in range(3):  # Find up to 3 paths
            ...
            for i in range(len(mask_pixels) - 1):
                ...
                segment = self.find_path_astar(cumulative_cost_map, segment_start_node, segment_end_node, combined_identity_map,
                                               width_map, main_vessel_width, temporal_cost_map, viz_callback=None)
        ...
        anim_data = {"type": "animation", "costmap": pathfinding_costmap, "pixels": mask_pixels,
                     ...
                     "temporal_cost_map": temporal_cost_map}
    ...
    def find_path_astar(self, cost_map, start, end, identity_map, width_map, main_vessel_width, temporal_cost_map, viz_callback=None):
        ...
        Args:
            ...
            temporal_cost_map: The map storing the frame index for each pixel.
        ...
        while open_set:
            ...
            for dr in [-1, 0, 1]:
                for dc in [-1, 0, 1]:
                    ...
                    # --- Directional Constraint: Prevent moving to an earlier frame ---
                    if temporal_cost_map is not None:
                        current_frame = int(temporal_cost_map[current[0], current[1]])
                        neighbor_frame = int(temporal_cost_map[neighbor[0], neighbor[1]])
                        if neighbor_frame < current_frame:
                            continue  # Do not allow moving backward in time
